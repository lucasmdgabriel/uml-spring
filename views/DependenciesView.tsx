...
export default function DependenciesView() {
    ...
    function changeDependencyClicked (i: number, j: number) {
        setDependencies(prev => prev.map((cat, ci) => {
            if (ci !== i) return cat;
            return {
            ...cat,
        ...
        <div className="w-full h-screen flex justify-center pt-[40px]">
            <div className="w-[90%] max-w-[1000px] flex flex-col gap-[48px]">
                <header className="flex flex-col">
                    ...
                    <a className="text-[16px]">Selecione as dependências que você quer adicionar ao projeto.</a>
                </header>
                        ...
                        dependencies.map((dependencyCategory, i) => {
                            return (
                                <div key={i} className="flex flex-col gap-4">
                                    <a className="text-[20px] font-bold">{dependencyCategory.name}</a>
                                    ...
                                    <div className="flex flex-row gap-4 flex-wrap">
                                        {
                                            dependencyCategory.dependencies.map((dependency, j) => {
                                                return (
                                                    ...
                                                    >
                                                        <div className={`border border-[#E5E7EB] w-[20px] h-[20px] rounded-[12px] mx-[17px] flex-shrink-0 ${dependency.clicked ? "bg-blue-300": undefined}`}/>
                                                        <div className="flex flex-col gap-1">
                                                            <a className="text-[#1D1D1F] font-bold text-[16px]">{dependency.name}</a>
                                                            <a className="text-[#6B7280] text-[14px]">Java annotation library which helps to reduce boilerplate code. Java annotation library which helps to reduce boilerplate code. Java annotation library which helps to reduce boilerplate code.</a>
                                                        </div>
                                                    </div>
                                                )
                                            })
                ...
                <footer className="flex flex-row justify-end gap-[12px] w-full bg-white">
